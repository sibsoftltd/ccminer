...
/***************************************************/
// Die Hash-Funktion
__global__ void _luffa512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector)
{
...
// Setup Function
__host__
void _luffa512_cpu_init(int thr_id, uint32_t threads)
{
...
__host__ void _luffa512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order)
{
    ...
    _luffa512_gpu_hash_64<<<grid, block, shared_size>>>(threads, startNounce, (uint64_t*)d_hash, d_nonceVector);
