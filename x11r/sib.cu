...
extern "C" int scanhash_sib(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (!init[thr_id])
	{
		...
		x11r_luffaCubehash512_cpu_init(thr_id, throughput);
		x11r_shavite512_cpu_init(thr_id, throughput);
		x11r_echo512_cpu_init(thr_id, throughput);
		if (x11r_simd512_cpu_init(thr_id, throughput) != 0) {
			return -1;
		}
	...
	do {
		...
		TRACE("gost   :");
		x11r_luffaCubehash512_cpu_hash_64(thr_id, throughput, d_hash[thr_id], order++);
		TRACE("luffa+c:");
		x11r_shavite512_cpu_hash_64(thr_id, throughput, pdata[19], NULL, d_hash[thr_id], order++);
		TRACE("shavite:");
		x11r_simd512_cpu_hash_64(thr_id, throughput, pdata[19], NULL, d_hash[thr_id], order++);
		TRACE("simd   :");
		x11r_echo512_cpu_hash_64(thr_id, throughput, pdata[19], NULL, d_hash[thr_id], order++);
		TRACE("echo => ");
